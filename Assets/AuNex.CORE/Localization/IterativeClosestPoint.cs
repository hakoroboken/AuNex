...
namespace AuNex
{
    namespace Localization
    {
        ...
        {   
            ...
            private bool setTargetPointsFlag = false;
            private float max_correspondence_distance = 0.3f;
            public ICP_Hash(float cellSize, float maxCorrespondenceDistance = 0.3f)
            {
                ...
                setTargetPointsFlag = false;
                max_correspondence_distance = maxCorrespondenceDistance;
            }
            ...
            public bool Align(List<Vector2> sourcePoints, int maxIterations = 20, double tolerance = 1e-4)
            {
                ...
                for (int iter = 0; iter < maxIterations; iter++)
                {
                    ...
                    foreach (var p in transformedPoints)
                    {
                        bool isFound = spatialHash.FindNearest(p, max_correspondence_distance, out var nearestPoint);
                        if(!isFound)
                        {
                            continue;
                        }
                        correspondences.Add(nearestPoint);
                    }
